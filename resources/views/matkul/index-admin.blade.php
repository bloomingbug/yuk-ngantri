            ...
            <table id="myTable" class="table table-bordered table-hover mt-3">
                ...
                <tbody>
                    @forelse ($matkul as $key => $item)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ strtoupper($item->kode_mk) }}</td>
                            <td>{{ $item->nama }}</td>
                            <td>{{ $item->dosenPengampu->name }}</td>
                            <td>{{ $item->semester }}</td>
                            <td>
                                <div class="d-flex flex-wrap flex-column flex-md-row justify-center">
                                    <form method="POST" action="/matkul/{{ $item->slug }}">
                                        @csrf
                                        @method('DELETE')
                                        <a href="/matkul/{{ $item->slug }}" class="btn btn-info btn-sm m-1">
                                            <i class="fa fa-info-circle" aria-hidden="true">
                                            </i>
                                        </a>
                                        <a href="/matkul/{{ $item->slug }}/edit" class="btn btn-warning btn-sm m-1">
